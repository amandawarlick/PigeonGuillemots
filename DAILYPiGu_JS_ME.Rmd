...
nMix <- function () {
  ...
  lambda[i] <- lambda.mu + b.upwell*upwell[i,j]
...
jags.data <- list(y = y, 
                  temp = as.matrix(temp[,2:6]), upwell = as.matrix(upwell[,2:6]), 
...
parameters <- c('lambda', 'p', 'N_est')
     
# MCMC settings
ni <- 1000; nt <- 1; nb <- 300; nc <- 3
     
out_nMix <- jags(jags.data, inits, parameters, model.file = model.file,
              n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = T)

#jagsUI::traceplot(out_nMix)
