...
#this doesn't quite get all the day combinations
all_days <- data_burrow_all %>%
  #filter(is.na(burrow_name)) %>%
  group_by(region, year, site) %>%
  distinct(yday)
# n_sites <- length(unique(data_burrow_all$site))
# sites <- unique(data_burrow_all$site)
...
# all_days <- data.frame(yday = rep(c(min_day:(min_day+max_days)), times = n_sites),
#                        year = rep(c(2008:2018), each = length(c(min_day:(min_day+max_days)))*n_sites),
#                        site = as.character(rep(sites, each = length(c(min_day:(min_day+max_days))))))
# all_days$site = as.character(all_days$site)

fill_full <- data.frame(year = rep(as.numeric(2020), times = length(c(min_yday:(min_yday+max_days)))),
                        burrow_name = as.character(rep('XXX', times = length(c(min_yday:(min_yday+max_days))))),
                        site = as.character(rep('Dummy', times = length(c(min_yday:(min_yday+max_days))))),
                        yday = c(min_yday:(min_yday+max_days)))
...
burrow_CH <- burrow %>%
  ...
  dcast(region + year + site + burrow_name + min_day + max_day ~ yday, value.var = 'capt_hist', 
        fun.aggregate = mean) %>%
  filter(!is.na(burrow_name)) %>%
  filter(site != 'Dummy') %>%
