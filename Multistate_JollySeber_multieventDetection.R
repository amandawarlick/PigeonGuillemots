...
Nstar <- 100                             # Superpopulation size (total number of burrows active on at least one day)
n.states <- 4                            # not entered, egg, chick, terminated
n.obs<-3                                 #burrow visit, prey delivery, not detected
b <- c(.5, seq(.2, .01, length=n.occasions-1))      # Entry probabilities decrease with time
alpha <- c(.40, rep(0,n.occasions-1))    # Prob burrow is a "chick burrow" given entry (here we assume this only occurs on day 1)
phiA<-.8                                 #survival state A (egg burrow)
phiB<-.7                                 #survival state B (chick burrow)
psiAB<-.25                               #transition A to B conditional on survival
pA<-.5                                   #detection for egg burrow
pB<-0.75                                 #detection for chick burrow
beta<-.25                                #conditional on observing a visit to a chick burrow, probability the delivery was a 'burrow visit'
...
simul.js_ms_me <- function(PHI.STATE, P.OBS, Nstar, n.occasions, n.obs, n.states){
   ...
   B <- rmultinom(1, Nstar, b) # Generate total number of entering burrows per occasion
   ent.occ <- numeric()
   for (t in 1:n.occasions){
