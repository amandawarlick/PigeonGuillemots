...
# no_start <- start_end %>%
#   filter(prey_start > bv_start | is.na(bv_start)) %>% nrow()/nrow(start_end)
# no_end <- start_end %>%
#   filter(prey_end == bv_start) %>% nrow()/nrow(start_end)
...
#after talking with Sarah, just want to create a capture history column with three observed states and NAs for unobserved 
...
burrow_full <- burrow_2 %>%
  ...
  transform(capt_hist = 
              ifelse(is.na(burrow_visit) & is.na(tot_prey), 'NA', 
                     ifelse(!is.na(tot_prey) & tot_prey > 0, 2,
                            ifelse(!is.na(burrow_visit), 1,
                       100))))
...
write.csv(burrow_full, "burrow_full_CH.csv", row.names = F)
...
burrow_1_week <- burrow %>%
  select(site, year, week, study_day, burrow_name, burrow_visit, tot_prey) 

# burrow_2_period <- burrow_1_period %>%
#   distinct(year, site, burrow_name) #%>%
#   #merge(all_days, all = T) 

burrow_full_week <- burrow_1_week %>%
  #merge(burrow_1_period, by = c('site', 'year', 'burrow_name', 'study_day'), all.x = T) %>%
  # merge(prey_weeks %>% select(year, site, burrow_name, prey_week_range), 
  #       by = c('year', 'site', 'burrow_name'), all.x = T) %>%
  merge(start_end_visits, by = c('year', 'site', 'burrow_name'), all.x = T) %>%
  #merge(exclusions, by = c('year', 'site', 'burrow_name'), all.x = T) %>%
...
hist(burrow_full_period$capt_hist)

write.csv(burrow_full_week, "burrow_full_week_CH.csv", row.names = F)
