...
#long version of capture history, spread out so there is one row per nest per day per year
...
#capture histories per day across columns
data_MEJS_wide <- data_MEJS %>% 
  distinct(region, year, site, burrow_name, study_day, capt_hist) %>% 
  #transform(capt_hist = ifelse(is.na(capt_hist), 4, capt_hist)) %>% #leave NAs for unobs days
  dcast(region + year + site + burrow_name ~ study_day, value.var = 'capt_hist', 
        fun.aggregate = mean)
data_MEJS_wide <- data_MEJS_wide[sample(nrow(data_MEJS_wide), 100),]

#matrix of just capture histories, 82 days
ch <- as.matrix(data_MEJS_wide %>% select(-c(region, year, site, burrow_name)), rownames.force = F)
colnames(ch) <- NULL
ch <- round(ch, 0)

## Create binary effort matrix with 1 if site surveyed on day t, 0 otherwise.
EFFORT_MEJS <- matrix(0, nr = dim(ch)[1], nc = dim(ch)[2])
EFFORT_MEJS[!is.na(ch)] <- 1
...
# 2 Prey visit
# 3 not seen
# 4 not observed - no survey that day
# -------------------------------------------------
...
for (i in 1:M){
 ...
 for (t in 2:n.occasions){
  ...
  y[i,t] ~ dcat(po[z[i,t], i, t-1,])  
   #this follow Kery and Schaub 10.3.2, indexing is tricky. e.g., y[i,2] is a function of state[i,2] and observation array t=1; if detection covariates are used, don't add dummy occasion to detection covariates as detection data at occasion t uses array t-1
...
# Define transition and observation matrices
 for (i in 1:M){
  for (t in 1:(n.occasions-1)) {
      ...
      ps[4,i,t,2] <- 0                        #probability terminated goes to egg (maybe this does happen)
      ps[4,i,t,3] <- 0                        #probability terminated goes to chick
      ps[4,i,t,4] <- 1                        #probability terminated goes to terminated
      
      # Define probabilities of O(t) given S(t)
      po[1,i,t,1] <- 0                        #probability a 'not entered' burrow is detected with a burrow visit
      po[1,i,t,2] <- 0                        #probability a 'not entered' burrow is detected with a prey visit
      po[1,i,t,3] <- 1                        #probability a 'not entered' burrow is not detected
...
# latent z
# all obs of 1,3,4 are unk state, NA
...
# z.st[i, 2:f[i]-1] <- 1
# }
z.st[,1] <- NA #this is set to 1 in the model, so need to revert to NA
...
for (i in 1:dim(z.st)[1]) {
  if (fpv[i] != Inf) {
  z.st[i, (fpv[i]+1):lpv[i]] <- 3 #chick between pv observations
  }
}
...
js.me.init <- function(ch, f, l){
  ...
  #inits[,1] <- NA  #deterministic on occasion 1; commented out or else indexing is out of bounds
...
# z.init <- rbind(zInitsObs, matrix(1, nr = nz, nc = dim(ch)[2]))
# z.init[,1] <- NA  #don't initialize dummy day

###### #simpler z.init ########
# everything that is deterministic gets NA (obs = 2).
# everything that is unknown (1, 3, NA) gets inits
## for unknown states:
#### 1 obs (bv) can get 2 (egg)
#### NA and 3 obs (not det) can get 1 (NE) before fc, 2 after first bv, 3 after first pv

#z.init <- ch
#z.init[which(z.init == 2)] <- NA #deterministic
# z.init[,1] <- NA #deterministic on dummy occasion
# z.init[which(z.init == 1)] <- 2 #bv observation can be egg state
# 
# for (i in 1:dim(z.init)[1]) {
#   z.init[i,2:(f[i]-1)] <- 1 #NE before first capture
#     if (fbv[i] != Inf & fpv[i] == -Inf) {
#       z.init[i,(fbv[i]-1):(lpv[i]-1)] <- 2 #egg after first bv
#     }
#   if (fbv[i] != Inf) {
#       z.init[i,(fbv[i]-1):(fpv[i]-1)] <- 2 #egg after first bv
#     }
#   if (fpv[i] != Inf) {
#   z.init[i, (fpv[i]):lpv[i]] <- NA #chick between pv observations, deterministic
#   }
# }
