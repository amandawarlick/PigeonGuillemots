...
me.init.z <- function(ch){
  inits <- ch
   for (i in 1:dim(inits)[1]) { 
     if (sum(inits[i,] == 2) < 1) {   #If no prey deliveries, all 1s or 3s
       inits[i,] <- rep(3, dim(ch)[2])
     } else {
    prey_first <- min(which(ch[i,] == 2)) #first prey delivery
    prey_last <- max(which(ch[i,] == 2)) #last prey delivery
    inits[i, prey_first:prey_last] <- NA
    inits[i, 1:(prey_first-1)] <- sample(1:2, prey_first-1, replace = T)
    inits[i, (prey_last+1):(dim(ch)[2])] <- sample(2:3, dim(ch)[2]-prey_last, replace = T)
     }
      }
   return(inits)
}
...
# Bundle data
jags.data <- list(y = CH_PG_day_small, 
                  f = f, 
