...
PG_count_tides <- data_count %>%
  ...
  select(region, site, date, PG_count, start_time, yday, year) %>%
  inner_join(tides %>% filter(region != 'SS'), by = c('site', 'date', 'region')) %>%
...
#uwpelling and temp at the monthly level
setwd("~/Documents/SAFS/PigeonGuillemots/WhidbeyData")
...
covs_cnt <- covs %>%
  select(year, site, yday, PG_count, v, type, from_hilo, temp, mo) %>%
...
covs_all <- covs_cnt %>%
  ...
  select(site, year, mo, yday, PG_count, bv, pv, type, v, from_hilo_orig, temp, up_anom)
covs_all[is.na(covs_all)] <- 0

write.csv(covs_all, 'covs_all.csv', row.names = F)
```
...
weekly_count_isl_trim_w <- week_perc_w %>%
  filter(perc_week > 0.699) %>%
  select(-c(max_cnt, perc_week)) %>%
  transform(region = 'Whidbey')
check_plot <- ggplot(weekly_count_isl_trim_w, aes(week, cnt, group = year, color = year)) +
...
weekly_count_col_trim_w <- week_perc_yr_w %>%
  filter(perc_week > 0.6999) %>%
  select(-c(max_cnt, perc_week)) %>%
  transform(region = 'Whidbey')
check_plot <- ggplot(weekly_count_col_trim_w, aes(week, PG_count, group = year, color = year)) +
  geom_line() + geom_point() +
  facet_wrap(~site) +
  plot_theme(legend.position = 'none')
