...
data <- data_PG %>%
  ...
  select(site, year, date, yday, week, PG_count, burrow_name, burrow_count, burrow_visits_day, burrow_visit,
         Gunnel, Sculpin, Other, tot_prey, intern_data) 
data <- data %>%
  transform(id = 1:nrow(data))


#adding on columns for number of weeks of prey deliveries
prey_weeks_range <- data %>% 
...
#I don't think I want the burrow-week-level summaries by date 
# data <- data %>%
#   merge(prey_weeks, by = c('year', 'site', 'burrow_name'))
...
write.csv(data, "data_combined.csv", row.names = F)
write.csv(data_burrow, "burrow_only.csv", row.names = F)
