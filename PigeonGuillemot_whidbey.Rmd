...
#fixing up website output for burrow-level data 2015-2017
burrow_2015_18 <- read.csv("burrow_data_2015_2018.csv", stringsAsFactors = F) %>%
  ...
  dcast(site + date + burrow_name + intern_data ~ visit_type, value.var = 'visits', 
        fun.aggregate = mean, fill = 0) %>%
  select(site, date, burrow_name, Gunnel, Sculpin, Other, burrow_visit, intern_data)


#read in early data (2008-2014) compiled by hand from spreadsheets

data_count <- read.csv("PGdata_pre_proc.csv", stringsAsFactors = F) %>%
...
tides <- read.csv("tides_tidy.csv", stringsAsFactors = F, header = T) %>%
  transform(date = as.Date(date)) %>%
  transform(month = month(date))

tides_counts <- tides %>%
  merge(data_count %>% filter(year > 2013 & intern_data != 'Y') %>%
          select(site, date, PG_count), by = c('site', 'date'), all.y = T) 
tides_counts_trim <- tides_counts %>%
  filter(month > 6 & month < 9)

mod <- lm(PG_count ~ from_low + site, data = tides_counts_trim)
summary(mod)

from_low <- ggplot(tides_counts_trim, aes(x = from_low, y = PG_count)) +
  #geom_point() + 
  geom_smooth(se = T) +
  xlab("Minutes from Low Tide" ) + ylab("PG Count") + 
  fig_theme()

from_low_col <- ggplot(tides_counts_trim, 
                       aes(x = from_low, y = PG_count, group = site, col = site)) +
  #geom_point() + 
  geom_smooth(se = F) +
  xlab("Minutes from Low Tide" ) + ylab("PG Count") + 
  fig_theme()

from_high <- ggplot(tides_counts_trim, aes(x = from_high, y = PG_count)) +
  #geom_point() + 
  geom_smooth(se = T) +
  xlab("Minutes from High Tide" ) + ylab("PG Count") + 
  fig_theme()

from_high_col <- ggplot(tides_counts_trim, 
                       aes(x = from_high, y = PG_count, group = site, col = site)) +
  #geom_point() + 
  geom_smooth(se = F) +
  xlab("Minutes from Low Tide" ) + ylab("PG Count") + 
  fig_theme()

#grid.arrange(from_low, from_high)

# ggplot(tides_counts_trim %>% filter(type.1 == 'H'), aes(x = tide.1, y = PG_count)) +
#   #geom_point() + 
#   geom_smooth(se = T)
# 
# ggplot(tides_counts_trim %>% filter(type.1 == 'L'), aes(x = tide.1, y = PG_count)) +
#   #geom_point() + 
#   geom_smooth()
