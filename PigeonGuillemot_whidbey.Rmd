...
burrow_allyrs <- test <- read.csv("burrow_pre_proc.csv", stringsAsFactors = F) %>%
  distinct() %>%
...
data_burrow <- burrow_allyrs %>%
  ...
  merge(count_dates, by = c('region', 'site', 'year', 'date', 'week', 'yday'), all = T) #check these get added on ALL burrow_name
...
# test <- data_burrow %>% distinct(site)
# test2 <- data_count %>% distinct(site)
# test3 <- burrow_allyrs %>% distinct(site)
# test4 <- count_dates %>% distinct(site)
...
ggplot(PG_count_tides_trim, aes(x = v, y = PG_count, col = type, group = type)) +
  #geom_point() + 
  geom_smooth(se = F) +
...
weeks_range <- ggplot(weeks_plot_data, 
  ...
  xlab("") + ylab("Number of Weeks") + scale_x_continuous(breaks = c(2008, 2010, 2012, 2014, 2016, 2017)) +
  facet_wrap(~site) + fig_theme()
...
trim_compare_plot <- ggplot(tot_compare, aes(as.numeric(as.character(year)), as.numeric(value), group = variable, col = variable)) +
