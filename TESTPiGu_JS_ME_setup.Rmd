...
data_burrow_all <- read.csv("data_burrow.csv", header = T, stringsAsFactors = F) %>%
  ...
  #filter(site %in% c('Mutiny Sands') & year == 2018) %>%
  #filter(site %in% c('Mutiny Sands', 'Double Bluff North') & year == 2018) %>%
  filter(year > 2017) %>%
  #dplyr::select(region, site, year, yday, burrow_name, burrow)
...
#   #dplyr::select(region, year, site, yday)
# name_expand <- data_burrow_all %>%
#   filter(!is.na(burrow_name)) %>% #use all unique burrow names per year to expand each per week
#   distinct(region, year, site, week, burrow_name) %>%
#   #merge(no_vis, all = T) %>%
#   merge(no_vis) %>%
#   #transform(capt_hist = 3) %>%
#   filter(!is.na(week)) %>% #remove cases where there aren't any missings, took week out since something was awry
#   dplyr::select(region, site, year, burrow_name, yday, week)
...
# year_burrow <- burrow %>%
#   transform(id = paste(burrow_name, year))
...
day_range <- burrow %>%
  group_by(region, year, site) %>%
  summarize(min_day = min(study_day), max_day = max(study_day))
...
#ledgewood 2016 is the long one, otherwise mostly 8-11 weeks - fixed
# test <- week_range %>%
...
burrow_CH <- burrow %>%
  ...
  select(region, year, site, week, yday, start_day, study_day, n_visits, 
         min_day, max_day, burrow_name, capt_hist) %>%
  distinct() %>% 
...
burrow_CH <- burrow_CH %>%
  ...
  dplyr::select(region, year, site, burrow_name, study_day, capt_hist) %>% #arrange(study_day)
...
max_recap <- dim(burrow_CH[1, -c(1:4)])[2] #max number of resights for ncol
...
max_visits <- dim(survey[1, -c(1:5)])[2]
