...
# read in data
data = read.table('titis2.txt', skip = 10) %>%
  select(-V7) %>%
  transform(sum = V1+V2+V3+V4+V5+V6) %>%
...
```{r message=FALSE, warning=FALSE}
devMULTIEVENT <- function(b,data,eff,e,garb,n_col,km1){
...
# likelihood
    l <- 0
    for (i in 1:n_col) # loop on ind
   {
      ei <- e[i] # date of first det
      oe <- garb[i] + 1 # init obs
      evennt <- data[,i] + 1 # add 1 to obs to avoid 0s in indexing
      ALPHA <- PI*BE[oe,] #initial state prob * first encounter observation +1 indexing
...
#order of params for devM: b,data,eff,e,garb,n_col,km1
library(microbenchmark)
res = microbenchmark(
  optim(binit,devMULTIEVENT,NULL,hessian=F,data,eff,fc,init.state,n_col,km1,method="BFGS"),
  do.call("optim", f),
  times=5
 )
 res2 = summary(res)
